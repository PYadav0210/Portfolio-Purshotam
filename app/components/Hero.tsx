...
const item: Variants = { hidden: { opacity: 0, y: 26 }, show: { opacity: 1, y: 0, transition: { duration: 0.9, ease: [0.22, 1, 0.36, 1] } } };
...
export default function Hero() {
  ...
  return (
    <section id="top" className="relative flex min-h-[100svh] flex-col items-center justify-center overflow-hidden px-5 py-24 text-center sm:px-6">
      ...
      <motion.div variants={container} initial="hidden" animate="show" className="relative z-10 flex w-full flex-col items-center">
        ...
        <motion.div variants={item} className="mb-6 flex h-5 items-center overflow-hidden">
          <AnimatePresence mode="wait">
            <motion.span key={roleIndex} initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -20, opacity: 0 }} transition={{ duration: 0.55, ease: [0.22, 1, 0.36, 1] }} className={`text-[10px] font-medium uppercase tracking-[0.3em] sm:text-[11px] sm:tracking-[0.4em] ${roleColor[roleIndex]}`}>{roles[roleIndex]}</motion.span>
          </AnimatePresence>
        </motion.div>
        ...
        <motion.h1
          ...
          className="w-full max-w-full bg-gradient-to-r from-violet-200 via-white via-30% to-cyan-200 bg-[length:200%_auto] bg-clip-text font-light leading-[1.05] tracking-[-0.03em] text-transparent"
          style={{ fontSize: "clamp(2.4rem, 12vw, 7rem)" }}
        >
        ...
        <motion.div variants={item} className="mt-9 flex items-center gap-5 text-[12.5px] text-zinc-500 sm:gap-6 sm:text-[13px]">
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.5, duration: 0.9 }} className="absolute bottom-8 z-10 hidden flex-col items-center gap-2 sm:flex">
