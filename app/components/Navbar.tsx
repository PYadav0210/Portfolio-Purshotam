...
export default function Navbar() {
  ...
  return (
    <>
      {/* Floating Jobs button — top-right, every page */}
      <motion.div initial={{ y: -30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.9, delay: 0.3, ease: [0.22, 1, 0.36, 1] }} className="fixed right-3 top-3 z-[60] sm:right-5 sm:top-5">
        <Link href="/jobs">
          <motion.div whileHover={{ scale: 1.05, y: -2 }} whileTap={{ scale: 0.96 }} className="group relative flex items-center gap-2 overflow-hidden rounded-full border border-emerald-400/40 bg-emerald-400/[0.12] px-4 py-2.5 backdrop-blur-xl shadow-[0_8px_30px_-6px_rgba(52,211,153,0.5),inset_0_1px_0_rgba(255,255,255,0.2)]">
            ...
            <span className="relative text-[12.5px] font-medium text-emerald-100 sm:text-[13px]">Live Jobs</span>
          </motion.div>
        </Link>
      </motion.div>
        ...
        <div className="mx-auto grid max-w-7xl grid-cols-[1fr_auto_1fr] items-center gap-3">
          <div className="flex justify-start">
            <motion.a href="/#top" initial={{ x: -30, opacity: 0 }} animate={{ x: 0, opacity: 1, backgroundColor: bg, borderColor: bd }} transition={{ duration: 0.9, ease: [0.22, 1, 0.36, 1] }} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.97 }} className={`${shell} group flex items-center gap-2.5 px-3 py-2 sm:px-4 sm:py-2.5`}>
              ...
              <span className="relative whitespace-nowrap text-[13px] font-medium tracking-tight text-zinc-100 sm:text-[13.5px]">Purshotam</span>
            </motion.a>
          </div>
          ...
          <div className="flex justify-end lg:hidden">
            <motion.button onClick={() => setOpen(!open)} aria-label="Toggle menu" initial={{ x: 30, opacity: 0 }} animate={{ x: 0, opacity: 1, backgroundColor: bg, borderColor: bd }} transition={{ duration: 0.9, delay: 0.08, ease: [0.22, 1, 0.36, 1] }} whileTap={{ scale: 0.92 }} className={`${shell} mt-14 flex h-11 w-11 flex-col items-center justify-center gap-1.5 sm:mt-0`}>
