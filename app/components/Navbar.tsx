    ...
    <header className="fixed inset-x-0 top-0 z-50 px-3 pt-3 sm:px-5 sm:pt-5">
      <div className="mx-auto grid max-w-7xl grid-cols-[1fr_auto_1fr] items-center gap-3">
        ...
        <motion.nav initial={{ y: -40, opacity: 0 }} animate={{ y: 0, opacity: 1, backgroundColor: bg, borderColor: bd }} transition={{ duration: 0.9, delay: 0.08, ease: [0.22, 1, 0.36, 1] }} className={`${shell} hidden items-center gap-0.5 px-2 py-2 lg:flex`}>
          ...
          <Link href="/jobs" className="relative ml-1 whitespace-nowrap rounded-full border border-emerald-400/30 bg-emerald-400/10 px-3.5 py-1.5 text-[12px] font-medium text-emerald-200 transition-colors hover:border-emerald-400/50 hover:text-emerald-100 xl:text-[12.5px]">
            Jobs
            <span className="ml-1.5 inline-flex h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-400 align-middle" />
          </Link>
        </motion.nav>
          ...
          <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.35, ease: [0.22, 1, 0.36, 1] }} className="mx-auto mt-3 max-w-7xl lg:hidden">
            <div className="relative overflow-hidden rounded-3xl border border-white/[0.12] bg-white/[0.07] p-3 backdrop-blur-xl shadow-[0_8px_40px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.16)]">
              ...
              <div className="relative grid grid-cols-2 gap-1.5">
                ...
                <Link href="/jobs" onClick={() => setOpen(false)} className="col-span-2 rounded-2xl border border-emerald-400/30 bg-emerald-400/10 px-4 py-3 text-center text-sm font-medium text-emerald-200 transition hover:bg-emerald-400/15">
                  Jobs — Live German tech jobs
                </Link>
              </div>
            </div>
          </motion.div>
